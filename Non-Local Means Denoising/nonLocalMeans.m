function [result] = nonLocalMeans(targetImage, sigma, h, patchSize, searchWindowSize)

%% Non-Local Mean Denoising - Integral
% Preallocate
    image = double(targetImage);
        ...
        for col = 1+delta:columns-delta
            ...
            for row_searchWindow = start_rows : end_rows
                for column_searchWindow = start_columns : end_columns
                    ...
                    distance = evaluateIntegralImage(integral_image, row_searchWindow, column_searchWindow, patchSize);
